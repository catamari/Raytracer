...
namespace
{
	...
	bool ScatterDielectric(const Material& mat, const Ray& ray, const HitRecord& hit, Color& outAttenuation, Ray& outScattered)
	{
		...
		Vec3 direction;
		if (!canRefract || Reflectance(cosTheta, refractiveIndex) > RandomValue())
		{
...
Material Material::MakeLambert(const Color& albedo)
{
	return Material
	{
...
Material Material::MakeMetal(const Color& albedo, double fuzz)
{
	return Material
	{
...
Material Material::MakeDielectric(double refractiveIndex)
{
	return Material
	{
